...
function tk.color.shift(color, hue, sat, val)
...
function tk.color.lum(color, br)
    local h, s, v, a = tk.color.hsv(color)
    local new_v = math.min(v * br, 1.0)
    local r, g, b, a = tk.color.hsv2rgb(h, s, new_v, a)
    return tk.color.rgba2hex(r, g, b, a)
end

function tk.color.set_color(color)
    local r, g, b, a = tk.color.rgba(color)
    return ImGui.ColorConvertDouble4ToU32(r, g, b, a)
end

function tk.push_color(flag, color)
    if tk.ctx and type(color) == 'string' or type(color) == 'table' then
        color = tk.color.set_color(color)
        ImGui.PushStyleColor(tk.ctx, flag, color)
    end
end

function tk.color.interpolate(start_color, end_color, progress)
    local start_r, start_g, start_b, start_a = tk.color.rgba(start_color)
    local end_r, end_g, end_b, end_a = tk.color.rgba(end_color)
    
    local start_h, start_s, start_v = tk.color.hsv(start_color)
    local end_h, end_s, end_v = tk.color.hsv(end_color)
    
    local h = start_h + (end_h - start_h) * progress
    local s = start_s + (end_s - start_s) * progress
    local v = start_v + (end_v - start_v) * progress
    local a = start_a + (end_a - start_a) * progress
    
    local r, g, b = tk.color.hsv2rgb(h, s, v)
    return tk.color.rgba2hex(r, g, b, a)
end

tk.color.theme_names = { --theme_var_descriptions_sorted
...
local function GetAllThemeColors(color_identifiers)
    local colors = {}
...
function tk.get_all_theme_colors()
    ...
    tk.all_theme_colors = GetAllThemeColors(tk.color.theme_names)
    return tk.all_theme_colors
end

...
function GetAllThemeColors(theme_path, color_identifiers)
    local colors = {}
    
    for _, color_id in ipairs(color_identifiers) do
      local colorval = reaper.GetThemeColor(color_id.k)
      if colorval and colorval ~= -1 then
        local hexcolor = string.format("%06x", colorval & 0xFFFFFF)
        colors[color_id.k] = hexcolor
      end
    end
    return colors
  end
  ...
  all_cols = GetAllThemeColors(theme_path, theme_var_descriptions_sorted)

  print(all_cols["col_main_bg2"])
